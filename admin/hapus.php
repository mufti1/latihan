...
if(isset($_post['rekening'])){
	...
	$cek = mysql_query("SELECT rekening FROM biodata WHERE rekening='$id'") or die(mysql_error());
	...
	if(mysql_num_rows($cek) == 0){
		...
		$del = mysql_query("DELETE FROM biodata WHERE rekening='$id'");
		
		//jika query DELETE berhasil
		if($del){
			
			echo 'Data siswa berhasil di hapus! ';		//Pesan jika proses hapus berhasil
			echo '<a href="admin.php">Kembali</a>';	//membuat Link untuk kembali ke halaman beranda
